...
<div class="row">
    <div class="col-md-4">
        @if($item->id>0)
        <a href="{{route('admin.badword.create')}}" class="btn btn-primary btn-block margin-bottom">@lang('app.admin.badword.create')</a>
        @endif
        <form role="form" method="post" action="{{$action}}" enctype="multipart/form-data">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                  @if($item->id>0)
                  <h3 class="box-title">@lang('app.admin.badword.update')</h3>
                  @else
                  <h3 class="box-title">@lang('app.admin.badword.create')</h3>
                  @endif
                  <div class="box-tools">
    ...
    <div class="col-md-8">
          @component('components.box', ['button'=>true, 'class'=>'box-primary'])
              ...
              <table class="table table-striped table-hover items-list">
                ...
                <tbody>
                    @if(count($items)>0)
                        @each('admin.badword.tr', $items, 'badword')
                    @else
