...
class Talk
{
     ...
     *
     * @param \App\Models\Thread $conversation
     * @param \App\Models\ChatMessage          $message
     */
    public function __construct(Thread $conversation, ChatMessage $message)
    {
    ...
    public function readMessage($messageId = null)
    {
        if (!is_null($messageId)) {
            $message = $this->message->with(['sender', 'thread'])->find($messageId);

            if ($message->thread->user_one == $this->authUserId || $message->thread->user_two == $this->authUserId) {
                return $message;
            }
        }
